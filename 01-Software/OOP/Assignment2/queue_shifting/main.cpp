...
void view_details() {
    switch (curr) {
        ...
        case New_Index:
            ...
            break;
        case Display_Index:
            // You can implement Display by index here if needed
            break;
        case Display:
            clrscr();
...
int check_emp_ind() {
    int ind = 0;
    int emp_id = -1;
    printf("\nEnter a valid employee ID please: ");
    _flushall();
    scanf("%d", &emp_id);
...
int check_added_emp_ind() {
    ...
    while (!valid) {
        ...
        for (ind = 0; ind < emp_ind; ind++) {
            if (Employee_IDs[ind] == emp_id) {
                printf("\nInvalid employee ID (already exists).\n");
                valid = 0;
                break;
            }
        }
...
int check_index() {
    ...
    while (1) {
        printf("\nEnter an employee INDEX to add (-1 to stop): ");
        _flushall();
        scanf("%d", &in_emp_ind);

        if (in_emp_ind == -1) {
            printf("\nDisplaying all employees by index:\n");
            printf("=====================================\n");
        ...
        if (in_emp_ind < 0 || in_emp_ind >= MAX_EMP) {
            printf("\nInvalid index. Please enter a value between 0 and %d.\n", MAX_EMP - 1);
            continue;
        }

        if (in_emp_ind < emp_ind) {
            printf("\nThis location is already dedicated to employee ID %d.\n", emp[in_emp_ind].id);
            continue;
        }
        ...
        printf("\nEmployee added successfully at index %d.\n", in_emp_ind);
    }
}
